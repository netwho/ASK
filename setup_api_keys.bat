...
echo ----------------------------------------
echo [AlienVault OTX]
if exist "%CONFIG_DIR%\OTX_API_KEY.txt" (
    echo ✓ API key file already exists: OTX_API_KEY.txt
    set /p UPDATE_CHOICE="Update this key? (y/N): "
    if /i not "!UPDATE_CHOICE!"=="y" (
        echo Skipping AlienVault OTX...
        echo.
        goto :summary
    )
)
echo Optional - Get your free API key at: https://otx.alienvault.com
set /p OTX_KEY="Enter AlienVault OTX API Key (or press Enter to skip): "
if not "!OTX_KEY!"=="" (
    echo !OTX_KEY! > "%CONFIG_DIR%\OTX_API_KEY.txt"
    echo ✓ AlienVault OTX API key saved
) else (
    echo Skipped AlienVault OTX (optional)
)
echo.

:abusech
REM Abuse.ch (URLhaus/ThreatFox)
echo ----------------------------------------
echo [Abuse.ch URLhaus/ThreatFox]
...
echo Optional - Get your free Auth-Key at: https://auth.abuse.ch
set /p ABUSECH_KEY="Enter Abuse.ch Auth-Key (or press Enter to skip): "
if not "!ABUSECH_KEY!"=="" (
    echo !ABUSECH_KEY! > "%CONFIG_DIR%\ABUSECH_API_KEY.txt"
    echo ✓ Abuse.ch Auth-Key saved
) else (
...
if exist "%CONFIG_DIR%\OTX_API_KEY.txt" (
    ...
    echo   ✗ AlienVault OTX (optional)
)

if exist "%CONFIG_DIR%\ABUSECH_API_KEY.txt" (
    echo   ✓ Abuse.ch (URLhaus/ThreatFox)
) else (
    echo   ✗ Abuse.ch (URLhaus/ThreatFox) (optional)
)
